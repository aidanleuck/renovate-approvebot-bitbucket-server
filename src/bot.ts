...
async function getAllProjects(): Promise<Project[]> {
  ...
  try {
    ...
    return (response.body as unknown as ApiResponse<Project>).values || [];
  } catch (error) {
  ...
  try {
    ...
    for (const projectKey of projectKeys) {
      ...
      try {
        ...
        const repositories = (
          (response.body as unknown as ApiResponse<Repository>).values || []
        ).map((repo) => ({
          ...repo,
          projectKey,
        }));
...
async function getPullRequestsForRepo(
  ...
  try {
    ...
    const allPrs =
      (response.body as unknown as ApiResponse<PullRequest>).values || [];
